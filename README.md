...
- **DMA-Capable GUI Text & Box Rendering:** Created `rg_gui_dma_malloc()` in `rg_gui.c` and updated all text rendering (`draw_text_center`, `draw_text_box`, `draw_text_line`, `draw_text_scaled`) and rounded box functions (`draw_rounded_box` across `app_music.c` and `app_files.c`) to allocate display buffers from internal DMA-capable memory (`MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL`). This completely eliminates `spi transmit (queue) color failed` (`ESP_ERR_INVALID_ARG`) DMA aborts during track transitions and UI navigation when internal DRAM is fragmented by audio pipeline tasks.
