...
- **Smart RTC Time Sync:** Injected a silent Javascript `window.onload` hook into the WAP captive portal. Upon opening the download page, your device seamlessly calculates its local Unix Timestamp (offset for your timezone) and hits a hidden `/sync_time` REST endpoint, automatically aligning the console's internal RTC clock with your exact local time.
